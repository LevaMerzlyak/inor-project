function Gallery(sSelector) {
	...
	var src = $(g.pans[0]).find('.gallery__img').attr('data-big');
	...
	g.show360 = function (pag) {
		...
		var src = pag.find('.gallery__img').attr('data-big');
	...
	g.fullScreen.on('click', function(event) {
		event.preventDefault();
		var iOS = !!navigator.platform && /iPad|iPhone|iPod/.test(navigator.platform);
		...
		if (iOS) {
			...
			g.fullScreen.removeClass('fullScreen__btn_out');
			...
			g.fullScreen.addClass('fullScreen__btn_out');
